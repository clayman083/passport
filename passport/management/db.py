...
def db(context):
    ...
    directory = conf['app'].get('migrations_root')

    db_conf = conf.get('postgres')
    db_uri = 'postgres://%s:%s@%s:%s/%s' % (
        db_conf.get('user'), db_conf.get('password'), db_conf.get('host'),
        db_conf.get('port'), db_conf.get('name')
    )
    ...
    context.migrations_config = config
